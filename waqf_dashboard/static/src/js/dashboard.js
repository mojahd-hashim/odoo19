...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const kanbanColor = c =>
    ({ green:'#2ECC8A', red:'#E85555', yellow:'#F0A500',
       orange:'#F0A500', grey:'#8FA3B3' }[c] || '#8FA3B3');
  const stateLabel = s =>
  ...
  const certPillClass = s =>
    ({ draft:'pending', submitted:'pending', consultant_review:'pending',
       ...
       rejected:'rejected', review:'pending', paid:'approved' }[s] || 'pending');
  ...
  const reviewStateLabel = s =>
    ({ pending:'لم يبدأ', submitted:'بانتظار الاستشاري',
       approved:'✓ معتمد', rejected:'✗ مرفوض', blocked:'🔒 مجمّد' }[s] || s);
  const docIcon = mime => {
    if (!mime) return '📁';
...
<div class="tab-panel" data-tab-panel="visits">
  ...
  window.open360Viewer = function(url, name) {
    if (!document.getElementById('pannellum-css')) {
      ...
      css.id = 'pannellum-css'; css.rel = 'stylesheet';
  ...
  function _show360(url, name) {
    ...
    if (!modal) {
      ...
      modal.id = 'modal-360'; modal.className = 'modal-overlay';
      modal.innerHTML = `
  ...
  chatInput?.addEventListener('keydown', e => {
    if (e.key === 'Enter' && !e.shiftKey) {
      e.preventDefault(); sendChat(chatInput.value);
    }
  });
