...
class WaqfConsultantController(http.Controller):
    ...
    def mosque_pending(self, mosque_id, employee=None, **kwargs):
        ...
        wo_submitted = request.env['contractor.work.order'].sudo().search([
            ('mosque_id', '=', mosque_id), ('state', '=', 'submitted')])
        wo_delivered = request.env['contractor.work.order'].sudo().search([
            ('mosque_id', '=', mosque_id), ('state', '=', 'delivered')])
        ...
        return api_response(data={
            ...
            'total_actions': (len(wo_submitted) + len(wo_delivered) +
    ...
    def work_order_detail(self, wo_id, employee=None, **kwargs):
        ...
        mosque = self._check_mosque_access(portal_user, employee, wo.mosque_id.id)
        if not mosque:
            return api_response(error='Access denied', status=403)
        ...
        return api_response(data={
            ...
            'task_name':       wo.task_id.name if wo.task_id else '',
            'boq_lines':       boq_lines,
            'site_photos':     [_photo_data(a) for a in wo.site_photo_ids],
            'delivery_photos': [_photo_data(a) for a in wo.delivery_photo_ids],
    ...
    def approve_commencement(self, wo_id, employee=None, **kwargs):
        ...
        wo.write({
            'state': 'approved',
            'commencement_approved_by': (
                portal_user.user_id.id if portal_user
                else employee.user_id.id if employee else False),
            'commencement_approval_date': date.today(),
        })
        self._msg(wo, f'✅ اعتمد البدء عبر التطبيق: {approver_name}')
    ...
    def reject_commencement(self, wo_id, employee=None, **kwargs):
        ...
        self._msg(wo, f'❌ رفض البدء عبر التطبيق: {rejector}<br/>السبب: {reason}')
    ...
    def grade_work_order(self, wo_id, employee=None, **kwargs):
        ...
        next_state = 'rework' if grade in ('c', 'd') else 'graded'
        wo.write({
            'grade':       grade,
            'grade_notes': notes,
            'graded_by':   (portal_user.user_id.id if portal_user
                            else employee.user_id.id if employee else False),
        ...
        if grade in ('c', 'd'):
            request.env['contractor.work.order.rework'].sudo().create({
                'work_order_id': wo.id,
                'grade':         grade,
                'notes':         notes,
                'date':          date.today(),
        ...
        self._msg(wo,
            f'📊 تقييم {grade.upper()} عبر التطبيق: {grader}'
    ...
    def mosque_submittals(self, mosque_id, employee=None, **kwargs):
        ...
        result = [{
            ...
            'photos':          [_photo_data(a) for a in s.document_ids],
        } for s in submittals]

        return api_response(data={'submittals': result, 'total': len(result)})
    ...
    def submittal_detail(self, sub_id, employee=None, **kwargs):
        ...
        return api_response(data={
            ...
            'photos':          [_photo_data(a) for a in sub.document_ids],
        })
    ...
    def approve_submittal(self, sub_id, employee=None, **kwargs):
        ...
        self._msg(sub,
            f'✅ اعتمدت العينة عبر التطبيق: {approver}'
    ...
    def reject_submittal(self, sub_id, employee=None, **kwargs):
        ...
        sub.action_reject()
        self._msg(sub,
            f'❌ رُفضت العينة عبر التطبيق: {rejector}<br/>السبب: {reason}')
    ...
    def close_work_order(self, wo_id, employee=None, **kwargs):
        ...
        closer = (portal_user.name if portal_user
                  else employee.name if employee else 'مستشار')

        new_state = 'warranty' if wo.warranty_ids else 'closed'
        wo.write({'state': new_state})
        self._msg(wo, f'🏁 تم إغلاق الأمر عبر التطبيق: {closer}')
